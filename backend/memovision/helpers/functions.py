...
def compute_chroma_from_audio(audio_array, fs=22050):
    ...
    f_chroma = get_chroma_features_from_audio(audio=audio_array, tuning_offset=tuning_offset)
    return f_chroma, tuning_offset


def compute_dtw_path(ref_chroma, target_chroma, ref_onset=None, target_onset=None, with_onsets=False):
...
def transfer_step_annotations(step, ref_duration, target_duration, wp):
    ...
    target_steps = list(scipy.interpolate.interp1d(wp[0] / 50, wp[1] / 50, kind='linear')(ref_positions))
