...
def rms(filename):
    ...
    if (request.method == 'PUT'):
        ...
        np.save(
            f'./user_uploads/{current_user.username}/{session.name}/{filename}/features/rms.npy',
            rms_array)
        measures = load_measures(current_user.username, session.name,
                                 track.filename, track.gt_measures)
        rms_measure = get_resampled_feature(track.length_sec,
                                            rms_array,
                                            measures,
                                            fps=10)
        np.save(
            f'./user_uploads/{current_user.username}/{session.name}/{filename}/features/rms_measure.npy',
            rms_measure)
        return jsonify({'message': 'success'})
    else:
        feature_list = get_feature_data(session.tracks,
...
def loudness(filename):
    ...
    if (request.method == 'PUT'):
        ...
        np.save(
            f'./user_uploads/{current_user.username}/{session.name}/{filename}/features/loudness.npy',
            loudness_array)
        measures = load_measures(current_user.username, session.name,
                                 track.filename, track.gt_measures)
        loudness_measure = get_resampled_feature(track.length_sec,
                                                 loudness_array,
                                                 measures,
                                                 fps=10)
        np.save(
            f'./user_uploads/{current_user.username}/{session.name}/{filename}/features/loudness_measure.npy',
            loudness_measure)
        return jsonify({'message': 'success'})
    else:
        feature_list = get_feature_data(session.tracks,
                                        ...
                                        measure_axis=True)
        if feature_list:
            relevance = compute_relevance(current_user.username,
                                          session,
                                          'loudness',
                                          fpm=10,
                                          downsample_method='var')
        else:
            relevance = {}
        return jsonify({
            'message': 'success',
